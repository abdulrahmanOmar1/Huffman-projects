...
public class Huffman {
	...
	public static void compress(File file) throws IOException {
		...
		// بجدد السايز تاع البفر
		byte[] buffer = new byte[1024];  		// Read file as (bytes)

		// counter
		int[] tmp = new int[256];
		//بقرا عدد الكركتر بالبايت الموجوده بالملف لحد 1024
		int size = scan.read(buffer, 0, 1024);
		int index = -1;
		do {
			for (int i = 0; i < size; i++) {
				//هون بروح يجيب الاسكي تاعت البايت اول حرف  لما i=0 وهيك
				//مثلا m=77
				index = buffer[i];
				if (index < 0) {
					index += 256;
				...
				if (tmp[index] == 0) {
					huffCodeArraySize++;
					} // Counting  number of different characters in the file
				//frrequnsy
				tmp[index]++;
			}
		...
		for (int i = 0; i < 256; i++)
			if (tmp[i] != 0) {
				//ex : char i convert 77 to m , tmp[i] to freqency of m
		...
		//get name file & add .huff to it
		outFileName = new StringTokenizer(file.getAbsolutePath(), ".").nextToken() + ".huff";
		//حط فيه اسم الفايل .huff
		File f = new File(outFileName);
		//بنشا فايل .huff
		// بجيب الفايل الي بدي اطبع عليه
		...
		for (int i = 0; i < huffCodeArraySize; i++) {
			...
			//بجيب الاشي سبيس او حرف وبخزن الاسكي الو
			outbuffer[tracker++] = (byte) huffCodeArray[i].character;
			if (tracker == 1024) {
				output.write(outbuffer);
				tracker = 0;
			}
			// بضيف عدد الكود لنكث تاعت الهاف(11001)
		...
		do {
			for (int i = 0; i < size; i++) {
				...
				if (index < 0)// If the Value was negative
					index += 256;
				for (int j = 0; j < out[index].length(); j++) {// out -->to store huffman code 
	...
	public static void deCompress(File infile) {
		try {
			...
			File file = new File(fileName);//نسخخ من فايل المضغوط
			
			
			
			FileInputStream scan = new FileInputStream(file);// to read bytes from a file

			byte[] buffer = new byte[1024];

			// Get The File Name
			//بقرا من الفايل من 0 ل 1024 وبخزن البفر
			size = scan.read(buffer, 0, 1024);//to read bytes from a file(outbuffer) into a buffer.
			char[] tmp = new char[200];
			while (flag) {
				if (buffer[tracker] != '\n')
					tmp[tracker++] = (char) buffer[bufferTracker++];//get the orginal path and store in tmp
			...
			bufferTracker++;
			originalFileName = String.valueOf(tmp, 0, tracker);//convert tmp(path.txt) to string 
			...
			nbChar = Integer.parseInt(String.valueOf(tmp, 0, tracker));//read first line after path
			bufferTracker++;
			...
			loopSize = Integer.parseInt(String.valueOf(tmp, 0, tracker));//read sceond line after path
			bufferTracker++;
			...
			for (int i = 0; i < loopSize; i++) {
				...
				//if bufferTracker equal to 1024 then read another buffer and set the (bufferTracker)
				if (bufferTracker == 1024) {
					size = scan.read(buffer, 0, 1024);
					bufferTracker = 0;
				}
